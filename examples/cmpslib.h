...
{
	LoggerOpen();
	fprintf(LGDATA.LF,"%sEnd--%s\n",LGDATA.INDENT_BUFFER,fn);
	...
	if (0 == LGDATA.INDENT_COUNT) fprintf(LGDATA.LF,"\n\n");
	LoggerClose();
}
...
template<class T>
void* GetAddress( T & in){return & in;}

template<class T>
void* GetAddress( T * in){return ((void*) in);}
...
inline string ValueAsString(T & in)
{
  ...
  ostringstream out;
  if (name=="int" )
	{ out << * ((int *) &in );return out.str();}

  if (name=="unsigned long" )
	{ out << * ((unsigned long *) &in );return out.str();}

  if (name=="long" )
	{ out << * ((long *) &in );return out.str();}

  if (name=="long long" )
	{ out << * ((long long *) &in );return out.str();}

  if (name=="unsigned long long" )
	{ out << * ((unsigned long long *) &in );return out.str();}
    
  if (name=="short" )
	{ out << * ((short *) &in );return out.str();}

  if (name=="unsigned short" )
	{ out << * ((unsigned short *) &in );return out.str();}

  if (name=="double" )
	{ out << * ((double  *) &in );return out.str();}

  if (name=="long double" )
	{ out << * ((long double  *) &in );return out.str();}

  if (name=="char" )
	{ out << * ((char  *) &in );return out.str();}

  if (name=="char *" )
	{ out << * ((char  *) &in );return out.str();}
  
  if (name=="char const*" )
	{ out << * ((char const *) &in );return out.str();}

  if (name=="unsigned char" )
	{ out << * ((unsigned char  *) &in );return out.str();}

  if (name=="bool" )
	{ out << boolalpha << * ((bool  *) &in );return out.str();}

  if (name=="std::string" )
	{ out << * ((std::string  *) &in );return out.str();}

  if (name=="std::istream" )
	{ out << "cant display value of std::istream" ;return out.str();}

  if (name=="std::ostream" )
	{ out << "cant display value of std::ostream" ;return out.str();}

  if (name=="std::ifstream" )
	{ out << "cant display value of std::ifstream" ;return out.str();}

  if (name=="std::ofstream" )
	{ out << "cant display value of std::ofstream" ;return out.str();}
...
inline string AsString() {return string("");} 
...
static void LoggerHigh(const char * fn, int ln, const char* format, Args...  args)
  ...
  void LoggerEndReturning(const char * fn,int ln,const T & val)
    {
    LoggerOpen();
    fprintf(LGDATA.LF,"%sEnd--%s at line %d\nReturn Value:%s",LGDATA.INDENT_BUFFER,fn,ln,ValueAsString(val).c_str());
    fprintf(LGDATA.LF,"\n\n");
    LoggerSetIndent(-2);
    LoggerClose();
    }
